function preprocessing_redpanda(subjectid)
    ...
    fn_behav = sprintf('%s_events.csv',prefix);
    fn_behav_tsv = sprintf('%s_events.tsv',prefix);
    ...
    assert(size(eventlist,1)==length(trl),...
        'Number of triggers should match number of events')
    ...
    t = [tempname '.csv'];
    writetable(neweventlist,t);
    movefile(t,fn_behav_tsv);

    %% Resample and epoch with fieldtrip
